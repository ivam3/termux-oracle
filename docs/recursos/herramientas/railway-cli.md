...
-   **Soporte para agentes de IA:** Railway CLI incluye soporte nativo para configurar agentes (OpenCode, Claude Code, etc.).

## Instalación

```bash
# Mediante el wrapper de i-HakLab:
apt install railway-cli
# O mediante el script oficial:
...
-   **Costos:** Railway opera con modelo de pago por uso. Revisa su [panel de precios](https://railway.app/pricing) para más detalles.
-   **Regiones:** Puedes especificar la región de despliegue con `railway region set`.
-   **Logs:** Visualiza logs en tiempo real con `railway logs`.
